...
def test_display_power():
    ...
    try:
        # Open I2C connection
        handle = lgpio.i2c_open(1, 0x3C)
        print("✓ Connected to display at 0x3C")
        
        # Test 1: Turn display ON with maximum contrast
        print("Turning display ON with max contrast...")
        lgpio.i2c_write_byte_data(handle, 0x3C, 0x00)  # Control byte
        lgpio.i2c_write_byte_data(handle, 0x3C, 0xAF)  # Display ON
        
        # Set maximum contrast
        lgpio.i2c_write_byte_data(handle, 0x3C, 0x00)  # Control byte
        lgpio.i2c_write_byte_data(handle, 0x3C, 0x81)  # Set contrast command
        lgpio.i2c_write_byte_data(handle, 0x3C, 0x00)  # Control byte
        lgpio.i2c_write_byte_data(handle, 0x3C, 0xFF)  # Max contrast
        ...
        lgpio.i2c_write_byte_data(handle, 0x3C, 0x00)  # Control byte
        lgpio.i2c_write_byte_data(handle, 0x3C, 0x21)  # Set column address
        lgpio.i2c_write_byte_data(handle, 0x3C, 0x00)  # Control byte
        lgpio.i2c_write_byte_data(handle, 0x3C, 0x7F)  # End column (127)
        
        lgpio.i2c_write_byte_data(handle, 0x3C, 0x00)  # Control byte
        lgpio.i2c_write_byte_data(handle, 0x3C, 0x22)  # Set page address
        lgpio.i2c_write_byte_data(handle, 0x3C, 0x00)  # Control byte
        lgpio.i2c_write_byte_data(handle, 0x3C, 0x03)  # End page (3)
        ...
        for i in range(512):  # 128x32 display = 512 bytes
            lgpio.i2c_write_byte_data(handle, 0x3C, 0xFF)  # All pixels ON
